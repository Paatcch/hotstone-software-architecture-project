...
public class HotStoneDrawing implements Drawing, GameObserver {
    ...
    private void createAndAddFiguresForGameState() {
        ...
        // [OK] TODO: Add the text figure for the shown player's power
        ...
        for (Card card : game.getHand(playerShown)) {
            System.out.println(":) card from createAndAddFiguresForGameState : " + card.getName());
    ...
    public void onPlayCard(Player who, Card card, int atIndex) {
        ...
        // [OK] TODO: Add another message if the card has an effect
        if(!card.getEffectDescription().equals("")){
            addMessage("... with effect: " + card.getEffectDescription());
        }
    ...
    public void onAttackHero(Player playerAttacking, Card attackingCard) {
        // [OK] TODO: Inform player
        addMessage("" + playerAttacking
                + " attacks " + Player.computeOpponent(playerAttacking)
                + " with " + attackingCard.getName() + ".");
    }
    ...
    public void onUsePower(Player who) {
        // [OK] TODO: Inform player
        addMessage(who + " used power: " + game.getHero(who).getEffectDescription());
    }
    ...
    public void onCardUpdate(Card card) {
        ...
        if (actor != null) {
            actor.updateStats();
            // [OK] TODO: update the stats of the card/minion
            addMessage(card.getName() + " has been updated.");
        }
    }

    @Override
    public void onCardRemove(Player who, Card card) {
        // [OK] TODO: Remove the minion and refresh field
        // NOTE: be SURE to use the
        // right internal data structure manipulation method
        // so both the figure collection AND the actorMap
        // is updated
        removeActorAndUpdateMapping(card);
        addMessage(who + "'s minion " + card.getName() + " is killed.");
    }

    @Override
    public void onHeroUpdate(Player who) {
        HeroFigure heroActor = heroMap.get(who);
        heroActor.updateStats();
        // [OK] TODO: Refresh the hero's stats
    }
    ...
    private void refreshHand(Player who) {
        ...
        if (who == playerShown) {
            ...
            for (Card card : game.getHand(who)) {
                ...
                System.out.println("card from refreshHand : " + card.getName());
                CardFigure actor = actorMap.get(card.getId());
                if(actor == null){
                    System.out.println("Actor is null");
                }else{
                    System.out.println("actor is not null");
                }
